...
Set the following variables on your server (systemd `EnvironmentFile`, shell export, or your process manager).
...
Use the Terraform deployment under `infra/terraform/`.
...
```bash
curl -X POST https://kejayacapo.shop/api/auth/signup \
...
```bash
curl -X POST https://kejayacapo.shop/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{
    "email": "test@example.com",
    "password": "SecurePass123"
...
- **POST** `/api/mpesa/callback` - M-Pesa callback (automatic)
...
### Check Logs
Check your server logs (systemd, pm2, docker logs, etc.).
...
**Solution:** Check that `SUPABASE_URL` and `SUPABASE_ANON_KEY` are set in your server environment
