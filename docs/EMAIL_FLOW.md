...
**File:** `api/utils/email-impl.js` → `generateTransactionEmailHTML()`
...
**File:** `api/utils/email-impl.js` → `generateOrderConfirmationHTML()`
...
**File:** `api/utils/email-impl.js` → `generateAdminNotificationHTML()`
...
**Check:**
1. `RESEND_API_KEY` is set correctly in your server environment
...
Solution: Add RESEND_API_KEY to your server environment
```
...
- [ ] `RESEND_API_KEY` in server environment
