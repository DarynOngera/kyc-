...
Set these environment variables in your server environment (local `.env`, systemd `EnvironmentFile`, or your process manager):
...
Deploy and restart your server process after changes.
...
- `api/mpesa/callback.js` - Payment processing
- `api/utils/supabase.js` - Database client
- `api/utils/email-impl.js` - Email service
...
2. **Monitor logs** - Check server logs regularly
